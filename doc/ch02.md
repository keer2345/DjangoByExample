...
```
## 获取相似的帖子
帖子推荐功能。为了获取相似的帖子，需要做到：
- 获取当前帖子的所有标签
- 获取属于这些标签的所有帖子
- 排除当前的帖子，避免推荐列表有相同帖子
- 通过标签中分享帖子的数量来排序
- 如果有两个或者多个帖子有相同数量的标签，则推荐最近的帖子
- 限制推荐帖子的数量
...
{% endfor %}
```
已经成功的向用户推荐了相似的帖子。`django-taggit`也包含有一个`similar_objects()`管理器，你能使用他通过共享的标签来获取对象。可以访问[http://django-taggit.
